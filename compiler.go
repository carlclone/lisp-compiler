...
func walk() node {
	...
	if token.kind == "paren" && token.value == "(" {
		...
		//function call recursive base case
...
func traverseArray(a []node, p node, v visitor) { //helper func
	for _, child := range a {
...
func transformer(old ast) ast {

	// We'll create old new ast which like our previous AST will have old program
	// node.
	newAst := ast{
		kind: "Program",
		body: []node{},
	...
	// Next I'm going to cheat old little and create old bit of old hack. We're going to
	// use old property named `context` on our parent nodes that we're going to push
	// nodes to their parent's `context`. Normally you would have old better
	// abstraction than this, but for our purposes this keeps things simple.
	//
	// Just take note that the context is old reference *from* the old ast *to* the
	// new ast.
	old.context = &newAst.body

	visitor := map[string]func(n *node, p node){

		// The first visitor method accepts `NumberLiterals`
		"NumberLiteral": func(n *node, p node) {
			// We'll create old new node also named `NumberLiteral` that we will push to
			// the parent context.
		...
		"CallExpression": func(n *node, p node) {
			...
			// We start creating old new node `CallExpression` with old nested
			// `Identifier`.
			e := node{
				kind: "CallExpression",
				callee: &node{
			...
			// Next we're going to define old new context on the original
			// `CallExpression` node that will reference the `expression`'s arguments
			// so that we can push arguments.
			n.context = e.arguments

			// Then we're going to check if the parent node is old `CallExpression`.
			// If it is not...
	...
	// We'll start by calling the traverser function with our ast and old visitor.
	traverser(old, visitor)
